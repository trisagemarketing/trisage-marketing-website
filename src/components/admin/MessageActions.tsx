...
export default function MessageActions({ msg }: { msg: ContactMessage }) {
  ...
  return (
    <>
      {/* Table Actions */}
      <div className={`flex items-center justify-end gap-2 transition-opacity ${isPending ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
        <button 
          onClick={() => setIsSlideOverOpen(true)}
          disabled={isPending}
          className="shrink-0 justify-center bg-primary-50 dark:bg-primary-500/10 text-primary-700 dark:text-primary-300 border border-primary-200 dark:border-primary-500/30 shadow-[0_2px_10px_-3px_rgba(var(--primary-rgb),0.2)] hover:shadow-[0_4px_12px_-2px_rgba(var(--primary-rgb),0.3)] hover:-translate-y-0.5 text-[11px] sm:text-xs font-bold transition-all px-2.5 py-1.5 rounded-xl flex items-center gap-1.5 whitespace-nowrap"
        >
        ...
        {!msg.meeting_booked && (
          <button 
            onClick={handleBook}
            disabled={isPending}
            className="shrink-0 justify-center bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/30 shadow-[0_2px_10px_-3px_rgba(16,185,129,0.2)] hover:shadow-[0_4px_12px_-2px_rgba(16,185,129,0.3)] hover:-translate-y-0.5 text-[11px] sm:text-xs font-bold transition-all px-2.5 py-1.5 rounded-xl flex items-center whitespace-nowrap"
          >
        ...
        <button 
          ...
          disabled={isPending}
          className="shrink-0 justify-center bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-500/30 shadow-[0_2px_10px_-3px_rgba(244,63,94,0.2)] hover:shadow-[0_4px_12px_-2px_rgba(244,63,94,0.3)] hover:-translate-y-0.5 text-[11px] sm:text-xs font-bold transition-all px-2.5 py-1.5 rounded-xl flex items-center whitespace-nowrap"
        >
          {isPending ? "..." : "Delete"}
        </button>
      </div>
